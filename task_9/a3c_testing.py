...
def worker(max_global_steps, global_model, optimizer, wid, lock, log_dir): 
    ...
    while global_step < max_global_steps // 4:  
        log_probabilities = []
        value_estimates = []    
        reward_list = []
...
# run_a3c method modeled after info in guard clause  
def run_a3c(num_workers=4, max_global_steps=2000, lr=1e-4, log_dir="a3c_logs"):          
    ...
    for wid in range(num_workers):  
        p = mp.Process(    
            target=worker,   
            args=(max_global_steps, global_model, optimizer, wid, lock, log_dir)  
        )  
        p.start()  
        processes.append(p)  
...
if __name__ == "__main__": 
    trained_model = run_a3c(num_workers=4, max_global_steps=2000, lr=1e-4, log_dir="a3c_logs")
